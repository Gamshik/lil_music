...
constexpr std::array<float, equalizer_chain::band_count> kBandFrequenciesHz{
    60.0F,
    170.0F,
    310.0F,
    600.0F,
...
std::size_t ramp_samples_for_rate(const float sample_rate) {
    return sample_rate > 0.0F
               ? static_cast<std::size_t>((sample_rate * static_cast<float>(kRampMilliseconds)) /
                                          1000.0F)
               : 0U;
}
...
void equalizer_chain::set_band_gains(const std::array<float, band_count>& band_gains_db) {
...
void equalizer_chain::process(float* interleaved_samples, const std::size_t frame_count) {
    ...
    for (std::size_t frame_offset = 0; frame_offset < frame_count; frame_offset += kControlBlockFrames) {
        ...
        float* processed_block =
            interleaved_samples + frame_offset * channel_count_;
        ...
        for (std::size_t sample_index = 0; sample_index < control_frames * channel_count_; ++sample_index) {
            // Level и auto-headroom относятся только к wet/EQ-ветке.
            // Когда эквалайзер выключен, dry-path должен проходить без этой дополнительной gain-обработки.
...
std::array<float, equalizer_chain::band_count> equalizer_chain::target_band_gains_db() const {
...
const std::array<float, equalizer_chain::band_count>& equalizer_chain::band_frequencies_hz() {
