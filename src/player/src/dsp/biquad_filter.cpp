...
void biquad_filter::process(
    ...
    const std::size_t channel_count) {
    if (interleaved_samples == nullptr || channel_count == 0 || z1_.size() != channel_count ||
        z2_.size() != channel_count) {
        return;
    }
    ...
    for (std::size_t frame_index = 0; frame_index < frame_count; ++frame_index) {
        ...
        for (std::size_t channel_index = 0; channel_index < channel_count; ++channel_index) {
            ...
            z1_[channel_index] = coefficients_.b1 * input - coefficients_.a1 * output +
                                 z2_[channel_index];
...
biquad_coefficients make_peaking_coefficients(
    ...
    // Верхние полосы должны оставаться ниже Nyquist, иначе коэффициенты становятся
    // слишком агрессивными и на некоторых потоках это слышится как артефакты.
